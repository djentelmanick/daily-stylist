...
deploy/                   # docker-compose: postgres, хранилище фото, распознавание, redis, rabbitmq
...
И RabbitMQ - через него планировщик передаёт отправщику, кому пора слать рекомендацию. Адрес - `RABBITMQ_URL` в `backend/.env`, там же логин и пароль; панель брокера - http://localhost:55673. Обменник и очередь заводят сами процессы при запуске.
...
3. Фронтенд:
   ...
   Утренняя рассылка - два отдельных процесса, боту они не нужны: `make sched` ставит задачи, `make sender` их разбирает и шлёт сообщения. Без них рекомендация по кнопке работает, а утром сама не приходит. Отправщику нужен адрес туннеля: из него собирается кнопка «Ещё варианты» под сообщением.
