...
// Штрафы складываются, лучший образ - с наименьшей суммой.
const (
	warmthStepPenalty    = 3
...
type Input struct {
	...
	// Сколько дней назад вещь надевали в последний раз: 1 - вчера.
...
// должны отличаться одеждой, а не только обувью.
...
func (recommender recommender) outerwearOptions() []*domain.Item {
	...
	// В тёплый дождь без зонта выручит непромокаемая куртка, если не слишком тёплая.
...
// options превращает вещи в варианты для перебора. Пустой список - один вариант «без вещи».
