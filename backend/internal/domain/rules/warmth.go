...
const (
	// Ниже этой температуры по ощущениям без верхней одежды холодно.
	outerwearBelow = 15.0
	// С этой температуры верхняя одежда днём уже жарит.
...
// conditions - что погода требует от образа.
type conditions struct {
	// Уровень теплоты по самой холодной части дня.
...
func conditionsFor(weather domain.Weather) conditions {
	return conditions{
		...
		precipitation: weather.PrecipitationChance >= rainChance,
		snow:          weather.Snow,
		windy:         weather.WindSpeed >= strongWind,
...
// Границы - те же, что в подписях уровней теплоты.
