...
func TestRecommend_ReturnsOutfitsWithTexts(t *testing.T) {
	...
	handler := NewHandler(testBotToken, failingWardrobe{}, recommender, service.NewLocations(&memoryLocations{}, stubCities{}))
...
func TestRecommend_MapsErrors(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			handler := NewHandler(testBotToken, failingWardrobe{}, &stubRecommender{err: test.err}, service.NewLocations(&memoryLocations{}, stubCities{}))
...
func TestSearchCities(t *testing.T) {
	...
	handler := NewHandler(testBotToken, failingWardrobe{}, &stubRecommender{}, service.NewLocations(&memoryLocations{}, cities))
	initData := signedInitData(testBotToken, 42, time.Now())
	...
	// Одна буква - ещё не запрос: сервис погоды не спрашиваем, отвечаем пустым списком.
...
func TestSetCity(t *testing.T) {
	...
	handler := NewHandler(testBotToken, failingWardrobe{}, &stubRecommender{}, service.NewLocations(repository, stubCities{}))
	initData := signedInitData(testBotToken, 42, time.Now())
...
func newHandler(wardrobe wardrobe) http.Handler {
	return NewHandler(testBotToken, wardrobe, &stubRecommender{err: errors.New("подбор не должен вызываться")}, service.NewLocations(&memoryLocations{}, stubCities{}))
}
