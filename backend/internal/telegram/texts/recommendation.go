...
func WeatherDetails(weather domain.Weather) []string {
	...
	switch {
	case weather.Snow:
		details = append(details, fmt.Sprintf("Снег, вероятность %d %%", weather.PrecipitationChance))
	case weather.PrecipitationChance >= showPrecipitationFrom:
		details = append(details, fmt.Sprintf("Дождь, вероятность %d %%", weather.PrecipitationChance))
	}
	if weather.WindSpeed >= showWindFrom {
		details = append(details, fmt.Sprintf("Ветер до %d м/с", int(math.Round(weather.WindSpeed))))
	}
	if weather.UVIndex >= showUVIndexFrom {
