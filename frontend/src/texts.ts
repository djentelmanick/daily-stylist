...
export const texts = {
  ...
  onboardingStep: (step: number, count: number) => `Шаг ${step} из ${count}`,
  skip: 'Пропустить',
  back: 'Назад',
  onboardingCityChosen: 'Можно оставить этот или найти другой ниже.',
  onboardingCityTitle: 'Где вы живёте?',
  onboardingCityHint: 'Подберу одежду по погоде в этом городе.',
  onboardingWardrobeTitle: 'Добавьте вещи',
  onboardingWardrobeHint: 'Начните с того, что носите чаще всего. Сфотографируйте вещь - форма заполнится сама.',
  onboardingBasics: 'Для первого образа нужны',
  onboardingAdded: (count: number) => `Добавлено: ${items(count)}`,
  next: 'Дальше',
  onboardingMorningTitle: 'Утренний образ',
  onboardingMorningHint: 'Каждое утро пришлю в чат образ на день. Время можно поменять и потом, в настройках.',
  showOutfit: 'Показать образ на сегодня',
  ...
  locate: 'Определить по геопозиции',
  locating: 'Определяю город',
  locationDenied: 'Нет доступа к геопозиции. Найдите город поиском или разрешите доступ.',
  locationUnavailable: 'Не получилось узнать геопозицию. Найдите город поиском.',
...
export const errorTexts: Record<ApiErrorCode, string> = {
  ...
  place_not_found: 'Рядом не нашлось города. Найдите его поиском.',
